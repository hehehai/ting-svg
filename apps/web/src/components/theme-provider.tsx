...
const Theme = ({
  ...
  const applyTheme = React.useCallback(
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Theme application requires conditional logic
    (selectedTheme: string | undefined) => {
      let resolved = selectedTheme;
      if (!resolved) {
        return;
      }

      // If theme is system, resolve it before setting theme
      if (selectedTheme === "system" && enableSystem) {
  ...
  const setTheme = React.useCallback(
    (newValue: string | ((prev: string | undefined) => string)) => {
      const newTheme =
...
export const script: (...args: unknown[]) => void = (
  attribute,
  storageKey,
  defaultTheme,
  forcedTheme,
  ...
  const classes =
    isClass && (value as ValueObject)
      ? (themes as string[]).map(
          (t: string | number) => (value as ValueObject)[t] || t
        )
      : (themes as string[]);
  ...
  function updateDOM(theme: string) {
    if (isClass) {
      ...
      el.setAttribute(attribute as string, theme);
    }
  ...
  function getSystemThemeInScript() {
  ...
  if (forcedTheme) {
    updateDOM(forcedTheme as string);
  } else {
    try {
      const themeName =
        localStorage.getItem(storageKey as string) || (defaultTheme as string);
